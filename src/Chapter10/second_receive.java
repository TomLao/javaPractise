...
public class second_receive {
	...
	public static void main(String[] args) throws Exception {
		...
		DatagramSocket dSocket = new DatagramSocket(8001); // 接收端ds对象,监听8001端口，调用receive()接收对象
		DatagramPacket dPacket = new DatagramPacket(bs, 1024);// 封装数据

		System.out.println("waiting");
		dSocket.receive(dPacket); // 等待接受DatagramPacket对象
									// 构建string来读取输出数据
		String str = new String(dPacket.getData(), 0, dPacket.getLength()) + " " + dPacket.getAddress() + " "
				+ dPacket.getPort();
